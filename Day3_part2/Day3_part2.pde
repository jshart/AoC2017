...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2017\\Day3_part1\\data\\mydata");
...
int[][] memory=new int[600][600];
int x=300;
int y=300;

public int sumOfNeighbours(int xt, int yt)
{
  int total=0;
...
public String dirString(Direction d)
{
    switch (d)
    {
      case RIGHT:
...
void setup() {
  ...
  do
  {
    switch (currentDir)
    {
      case RIGHT:
        ...
        {
          x++;
          memory[x][y]=sumOfNeighbours(x,y);
        }
        break;
      case UP:
    ...
    //print(" CA:"+currentAddress);
    //print(" X:"+x+" Y:"+y);
    //println(" MEM:"+memory[x][y]);
  ...
  //println("**** Exit state:"+dirString(currentDir));
  //println("CA:"+currentAddress);
  //println("TA:"+targetAddress);
  //println("VD:"+vOffset+" HD:"+hOffset+" LO:"+(targetAddress-currentAddress));
  
  switch (currentDir)
  {
    case RIGHT:
      vOffset+=(targetAddress-currentAddress);
      break;
    case UP:
      hOffset-=(targetAddress-currentAddress);
      break;
    case LEFT:
      vOffset-=(targetAddress-currentAddress);
      break;
  ...
  //println("Fixed VD:"+vOffset+" HD:"+hOffset);
  //println("Answer:"+(abs(vOffset)+abs(hOffset)));
