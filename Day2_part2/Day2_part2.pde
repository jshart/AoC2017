...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2017\\Day2_part2\\data\\mydata");
...
void setup() {
  ...
  {
    total+=calcAnswer(input.lines.get(i));
    println("L:"+input.lines.get(i)+" T:"+total);
  }
}

public long calcAnswer(String s)
{
  ...
  for (i=0;i<numbers.size();i++)
  {
    ...
    {
      a=numbers.get(i)/numbers.get(j);
      b=a*numbers.get(j);
      if (b==numbers.get(i))
      {
        println(numbers.get(i)+" can be divided exactly by "+numbers.get(j));
        return(a);
      }
    }
  }
